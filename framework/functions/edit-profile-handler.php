    ...
    function inspiry_update_profile() {
        ...
        if( wp_verify_nonce( $_POST['user_profile_nonce'], 'update_user' ) ) {
            ...
            if ( !empty( $_POST['facebook-url'] ) ) {
             ...
             // Update vkontakte url
            if ( !empty( $_POST['vkontakte-url'] ) ) {
                $vkontakte_url = sanitize_text_field( $_POST['vkontakte-url'] );
                update_user_meta( $current_user->ID, 'vkontakte_url', $vkontakte_url );
            } else {
                delete_user_meta( $current_user->ID, 'vkontakte_url' );
            }
